...
#   4. Release notes collection
#   5. Git operations (commit, annotated tag, push)
#   6. Tag cleanup (delete all previous tags and GitHub releases)
#   7. CI/CD trigger (tag push triggers GitHub Actions)
#
...
git add $cargoToml
# Also stage Cargo.lock if it was regenerated by the build step
$cargoLock = Join-Path $projectRoot "Cargo.lock"
if (Test-Path $cargoLock) { git add $cargoLock }

# Check whether there is actually anything staged to commit
$staged = git diff --cached --name-only
if (-not $staged) {
    Write-Warn "No changes to commit (version may already be $Version in git)"
} else {
...
# ── Step 7: Push (triggers CI/CD) ───────────────────────────────────────

Write-Step 7 "Push to Origin (triggers CI)"
